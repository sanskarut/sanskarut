...
export async function checkRateLimit(req, res, next) {
    try {
        ...
        const user = await User.findById(userId).select("plan apiCallCount apiCallLimit apiCallResetAt");
        if (!user) {
            res.status(401).json({
                success: false,
                error: "Unauthorized",
        ...
        if (now >= user.apiCallResetAt) {
            ...
            console.log(`[Rate Limiter] Monthly reset applied for user ${userId}. ` +
        ...
        if (user.apiCallCount >= user.apiCallLimit) {
            ...
            res.status(429).json({
                ...
                message: `You have used ${user.apiCallCount}/${user.apiCallLimit} API calls on your ${planLabel} plan. ` +
        ...
        console.log(`[Rate Limiter] User ${userId} (${user.plan}): ` +
